...
## 2017년도 1월 ~ 6월
url_2017 = 'https://search.naver.com/search.naver?date_from=20170101&date_option=8&date_to=20170630&dup_remove=1&nso=p%3Afrom20170101to20170630&post_blogurl=&post_blogurl_without=&query="남는시간"&sm=tab_pge&srchby=all&st=sim&where=post&start='
...
## 2018년도 1월 ~ 6월
url_2018 = 'https://search.naver.com/search.naver?date_from=20180101&date_option=8&date_to=20180630&dup_remove=1&nso=p%3Afrom20180101to20180630&post_blogurl=&post_blogurl_without=&query="남는시간"&sm=tab_pge&srchby=all&st=sim&where=post&start='
...
text_conf = final_data[c(which(is.finite(increasing_rate) & increasing_rate > 4), which(is.finite(decreasing_rate) & decreasing_rate > 4.5)),]
str(text_conf)
text_conf$keyword
loc_conf = decreasing_rate[c(which(is.finite(increasing_rate) & increasing_rate > 4.5), which(is.finite(decreasing_rate) & decreasing_rate > 5))][1]
# 위치를 저장하려는 목적. 
text_conf$keyword
# ---------------------------
plot(final_data$decreasing_rate, final_data$increasing_rate, ylim = c(0, 10),
     xlim = c(0, 10), ylab = 'Increased ratio in 2018', xlab = 'Decreased ratio in 2018', main = 'Keyword Ratio')

text(1, 8.5,
     labels = '직장인', cex = 1, pos = 3)
text(1.3, 7.5,
     labels = '아르바이트', cex = 1, pos = 3)
# 남편
text(0.9,5.6, 
     labels = '남편', cex = 1, pos = 3)
# 인천공항, 
text(0.9, 5,
     labels = '집안', cex = 1, pos = 3)

text(1.2, 4.5,
     labels = '인천공항', cex = 1, pos = 3)
