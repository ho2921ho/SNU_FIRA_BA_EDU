...
sample (1:40 ,5)
sample (c("H","T"), 10, replace =T) #복원추출
sample (c("s", "f"), 10, replace =T, prob =c(0.9 , 0.1)) # 각 값이 나올 확률 지정.
...
par (mfrow = c(1 ,1))
x <- runif(50,0,2); y <- runif(50,0,2) #random sample in uniform(0,2) distribution 50개씩 추출.
...
hist.with.normal <- function (x, xlab = deparse (substitute(x)) ,...){ # deparse 모르는 자료형을 캐릭터형으로 바꿔준다. # ...은 히스토그램의 arg를 생략하는 것. 없어도 돌아간다. 
                                                                      # as.charactrer는 안 될 수도 있다. 
      h <- hist (x, plot =F, ...)
    s <- sd(x)
    m <- mean (x)
